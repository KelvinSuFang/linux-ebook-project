...
typedef struct PageDesc {
	int iPage;
	unsigned char *pucLcdFirstPosAtFile; // LCD上第一个字符在文件中的位置
	unsigned char *pucLcdNextPageFirstPosAtFile;// LCD上下一页的第一个字符在文件中的位置
	// 双向链表结构
	struct PageDesc *ptPrePage;
	struct PageDesc *ptNextPage;
} T_PageDesc, *PT_PageDesc;

static int g_iFdTextFile;	// 打开txt文本的fd
static unsigned char *g_pucTextFileMem;	// 文本fd的mmap的映射的内存.
static unsigned char *g_pucTextFileMemEnd; // 文本fd的mmap的映射的内存的结束地址
static PT_EncodingOpr g_ptEncodingOprForFile; //为这个文件选择一个编码器

static PT_DispOpr g_ptDispOpr;

// lcd上文件的第一个字符的第一个位置,在打开文件时设置
static unsigned char *g_pucLcdFirstPosAtFile; 
static unsigned char *g_pucLcdNextPosAtFile;

static int g_dwFontSize;	// 全局变量,字体大小

static PT_PageDesc g_ptPages   = NULL;
static PT_PageDesc g_ptCurPage = NULL; // 当前页
...
int SetFontDetail(char *pcHZKFile, char *pcFileFreetype, unsigned int dwFontSize)
{
	...
	while (ptFontOpr)
	{
		...
		if (iError == 0)
		{
			 ...
			 * 所以只要有一个FontInit成功, SetFontDetail最终就返回成功
			 */
			iRet = 0;
		}
		else
...
int ShowNextPage(void)
{
	int iError;
	PT_PageDesc ptPage;
	unsigned char *pucTextFileMemCurPos;  // 当前位置
